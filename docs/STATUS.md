...
Last build tag: **`[b34-live]`**. Target: Windows XP SP3 32-bit NTVDM. Built on macOS with Homebrew MinGW-w64 i686, CRT-free, no XP-incompatible deps.
...
3. **SB digital PCM/DMA**: **8-bit plays the correct sounds on the VM** (`[b33-wavdump]`, Skyroads). The path that got there (all verified by trace + ear):
   ...
   - **NEXT SESSION — hypotheses/steps (ranked):**
     1. **Capture the bytes we actually feed.** Add a one-shot dump of the first ~32 bytes passed to `audio_pcm_feed` (post signed→unsigned convert). If those are smooth PCM, the bug is in the DS streaming buffer mechanics; if jagged, the bug is upstream in how we read/convert. This is the missing measurement — we dumped the *guest snapshot* (empty) but never the *fed* bytes.
